...
public interface PowerDataRepository extends JpaRepository<PowerData, Integer> {
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<PowerData> findHourlyUsage(
    ...
    @Query(value = """
            SELECT DATE(recorded_at) as date,
                   SUM(real_usage_kwh) as `usage`
            FROM PowerData
            WHERE user_id = :userId
            AND MONTH(recorded_at) = :month
            AND YEAR(recorded_at) = :year
            GROUP BY DATE(recorded_at)
            ORDER BY date
            """, nativeQuery = true)
    List<Map<String, Object>> findDailyUsage(
            @Param("userId") Integer userId,
    ...
    @Query(value = """
            ...
            ORDER BY month
            """, nativeQuery = true)
    List<Map<String, Object>> findMonthlyUsage(
            @Param("userId") Integer userId,
            @Param("year") Integer year
    );
}